...
### 3.3 Release notes — v1.10.1+20 (current)

**Slovene** (max 500 chars):
```
Različica 1.10.1 (build 20)
• Popravek: po brisanju zapisa je ikona za sinhronizacijo še vedno kazala, da je nekaj neusklajenega, čeprav je bil zapis na strežniku že izbrisan. Ročna sinhronizacija ni več potrebna.
• Enak popravek velja za brisanje obhoda.
```

**English** (max 500 chars):
```
Version 1.10.1 (build 20)
• Fix: after deleting a record, the sync icon still showed something as out of sync even though the record had already been deleted on the server. A manual sync is no longer needed.
• The same fix applies to deleting a patrol (obhod).
```

### 3.3a Release notes — v1.10.0+19 (historical, superseded)
...
### 3.3b Release notes — v1.9.0+18 (historical, superseded)

**Slovene** (max 500 chars):
```
Različica 1.9.0 (build 18)
• Svoj zapis lahko zdaj izbrišete: v „Seznamu zapisov" se pri zapisu dotaknite gumba s tremi pikami in izberite „Izbriši zapis".
• Brisanje deluje tudi brez signala. Zapis takoj izgine s seznama in z zemljevida, na strežniku pa se izbriše ob naslednji sinhronizaciji — vmes se ne vrne nazaj.
• Zapisa, ki ga je pisarna že prevzela v obravnavo, ni mogoče izbrisati. Aplikacija pove, zakaj.
```

**English** (max 500 chars):
```
Version 1.9.0 (build 18)
• You can now delete your own record: in "Seznam zapisov", tap the three-dot button on a record and choose "Izbriši zapis".
• Deleting works without a signal. The record disappears from the list and the map straight away and is removed on the server at the next sync — it will not come back in the meantime.
• A record the office has already taken into review cannot be deleted. The app explains why.
```

### 3.3c Release notes — v1.8.0+17 (historical, superseded)

**Slovene** (max 500 chars):
```
Različica 1.8.0 (build 17)
• Pred shranjevanjem zapisa je nov korak: gumb „Preglej in shrani" najprej odpre predogled, kjer zapis vidite tako, kot bo videti v podrobnostih — fotografije, tipi motnje, lokacija in čas.
• V predogledu se z „Uredi" vrnete v obrazec z vsemi že vnesenimi podatki, z „Shrani zapis" pa zapis shranite.
• Popravek: dvojni dotik gumba za shranjevanje je lahko ustvaril dva enaka zapisa. Odslej se shrani samo eden.
```

**English** (max 500 chars):
```
Version 1.8.0 (build 17)
• A new step before a record is saved: "Preglej in shrani" opens a preview showing the record exactly as it will look in the details view — photos, disturbance types, location and time.
• From the preview, "Uredi" returns you to the form with everything you entered still there; "Shrani zapis" saves the record.
• Fix: double-tapping the save button could create two identical records. Now only one is saved.
```

### 3.3d Release notes — v1.7.0+16 (historical, superseded)

**Slovene** (max 500 chars):
```
Različica 1.7.0 (build 16)
• Podrobnosti motnje imajo nov razdelek „Obravnava": status, kdo je zadevo obravnaval in kdaj.
• V „Seznamu zapisov" je ob datumu barvni status obravnave, pod njim pa obhod, med katerim je bil zapis narejen — dotik odpre ta obhod.
• Pike na zemljevidu obhoda so obarvane po statusu, enako kot na glavnem zemljevidu.
• Filter „Starost" je odstranjen; obdobje izberete z drsnikom „Obdobje".
```

**English** (max 500 chars):
```
Version 1.7.0 (build 16)
• Disturbance details have a new "Obravnava" section: status, who handled the case and when.
• In "Seznam zapisov" each record shows a colour-coded status next to the date, and below it the patrol it was recorded during — tap to open it.
• Dots on the patrol map are coloured by status, matching the main map.
• The "Starost" filter is gone; pick a period with the "Obdobje" slider.
```

### 3.3e Release notes — v1.6.0+15 (historical, superseded)

**Slovene** (max 500 chars):
```
Različica 1.6.0 (build 15)
• Pike na zemljevidu so odslej obarvane po statusu obravnave in ne več po starosti zapisa: oranžna – odprto, modra – v obravnavi, zelena – zaključeno, siva – predano drugi službi.
• V oknu „Motnje" je nov razdelek „Status obravnave". Z njim izberete, kateri statusi so vidni na zemljevidu, hkrati pa je razdelek legenda barv.
• Pri podrobnostih motnje je zdaj vidno, kdo je zadevo obravnaval in kdaj.
```

**English** (max 500 chars):
```
Version 1.6.0 (build 15)
• Map dots are now coloured by case status instead of record age: amber – open, blue – in progress, green – closed, grey – handed to another service.
• The "Motnje" panel has a new "Status obravnave" section. Use it to choose which statuses appear on the map; it also doubles as the colour legend.
• A disturbance's details now show who handled the case and when.
```

### 3.3f Release notes — v1.5.0+14 (historical, superseded)
...
### 3.3g Release notes — v1.4.0+13 (historical, superseded)
...
### 3.3h Release notes — v1.3.1+12 (historical, superseded)
...
### 3.3i Release notes — v1.3.0+11 (historical, superseded)
...
### 3.3j Release notes — v1.2.3+10 (historical, superseded)
...
### 3.3k Release notes — v1.2.0+7 (historical, superseded)
...
### 3.3l Release notes — v1.1.3+6 (historical, superseded)
...
### 3.3m Release notes — v1.0.1+2 (historical, superseded)
