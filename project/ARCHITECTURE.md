...
### 10.2 Overlay: Protected Areas (Natura 2000, WMS)
Optional overlay of Natura 2000 protected areas ("Območja s statusom"), toggled by the "Območja" chip in the home-map layer row. Server-rendered **WMS raster tiles** — the same mechanism as the Parcele overlay — with tap-to-identify via WMS GetFeatureInfo.

- **Source**: the production NarcIS **GeoServer** (the one the APEX NarcIS app itself uses) at `https://narcis.gov.si/ows/ows` — public WMS, no auth for these layers. The "Območja s statusom" layers live in the `SI.NARCIS` workspace; this overlay uses `SI.NARCIS:ZOS_N2K_PLG` (Title "Natura 2000"). The same WMS also publishes `ZOS_EPO_PLG`/`ZOS_EPO_PNT`, `ZOS_NV_PNT_JAME`, etc. for when those are added.
- **Why WMS, not vector** (decided 2026-06-15): the earlier vector approach (GeoJSON from the ORDS `vib/zos` module, reprojected client-side via `proj4dart`) was abandoned because that ORDS endpoint serialises the layer live (`SDO_UTIL.TO_GEOJSON`) — **~14–16 s time-to-first-byte per cold request** (measured), and a whole-layer download + CPU render does not scale to the larger ZOS layers (`NV` 5.3k, `NVJ` 14.7k features). WMS tiles load per-viewport: first paint is immediate and it scales to any layer because GeoServer rasterises server-side. (The `narcis-vibed` web app caches the ORDS GeoJSON in Next.js — `narcis.alittis.com/api/layers/:tip`, ~1 s — but the field app deliberately does not depend on that deployment.)
- **Tiles (GetMap)**: WMS 1.3.0, `EPSG:3857`, `image/png` transparent, `STYLES=` empty → GeoServer's own published SLD (the official Natura symbology comes from the server). `obmocjaWmsLayers()` in `lib/widgets/basemap.dart` returns the `TileLayer` (mirrors `parceleOverlayTileLayers`); `maxZoom: 19`, visible at all zooms.
- **Tap-to-identify (GetFeatureInfo)**: `MapOptions.onTap` → `ObmocjaStore.identify(LatLng)` issues a WMS 1.3.0 GetFeatureInfo (`EPSG:4326`, small bbox around the tap, centre pixel + `BUFFER`, `INFO_FORMAT=application/json`, `FEATURE_COUNT=10`). SPA (POV) and SAC (POO) overlap the same ground, so a tap usually returns several features — the one call yields **both the list and the full per-area attributes** (`IME_OBM`, `KODA_OBM`, `N2K_TIP_OBMOCJA`, `N2K_BIOGEO_REGION`, `POV_HA`, `OPIS`, dates). No separate detail endpoint, no client reprojection.
- **Sheet**: `showObmocjaSheet` lists the hits (POV/POO colour dot + name + code); a row-tap drills into that area's detail (a single hit opens detail directly). All data is already in the GetFeatureInfo response — the sheet does no further fetching.
- **State**: `_showObmocja` (widget-local, not persisted) toggles the tile layer instantly — no load step. `_identifying` shows a small chip while a tap's GetFeatureInfo is in flight; a tap on no area does nothing; a network failure shows a snackbar.
- **Scope**: N2k only for now; adding `EPO`/`NV`/`jame` is a few lines each (another `TileLayer` + chip + the layer name in GetFeatureInfo). `proj4dart` and the GeoJSON parsing were removed with the vector approach.
- Implementation: `lib/data/obmocja_store.dart` (`identify` GetFeatureInfo + `ObmocjeFeature`), `obmocjaWmsLayers()` in `lib/widgets/basemap.dart`, `lib/widgets/obmocje_sheet.dart` (list ⇄ detail), wired in `home_screen.dart` (`_showObmocja`, `_identifyObmocja`, the "Območja" chip). Unit tests in `test/obmocja_store_test.dart` (GetFeatureInfo request shape + overlapping-feature parse).
