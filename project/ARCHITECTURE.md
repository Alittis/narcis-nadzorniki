...
- **⚠️ Content-Type is per-verb, and a bodyless request must NOT declare one (TB-33).** ORDS parses the request payload for a body-bearing method as soon as a JSON content type is declared, and an empty body is not valid JSON — so a bodyless `DELETE` sent with `Content-Type: application/json` is rejected by **ORDS itself, before the PL/SQL handler runs**, with `400 {"code":"BadRequest","message":"Expected one of: <<{,[>> but got: <<EOF>>"}`. Note 400 is not in the DELETE row of the table above, because the handler cannot produce it. Verified against production 2026-09-02: the same `DELETE` returns 401 (i.e. reaches the handler) **without** the header and 400 **with** it. `RemoteApi._authHeaders` (auth + `Accept`) serves GET/DELETE; `_jsonHeaders` adds `Content-Type` and serves POST/PUT only.
  - **Why this stayed latent for months.** `GET` survives it — ORDS does not attempt payload parsing on GET — and the disturbance `DELETE` had no UI path until TB-2, so nothing exercised a bodyless Dart request. [tools/ords/test_disturbances.sh](../tools/ords/test_disturbances.sh) adds `Content-Type` only when it actually sends a body, so the endpoint's own smoke test passed throughout and §9.3 could honestly call DELETE "smoke-tested" while the client was broken. **A curl smoke test passing is not evidence the Dart client's request shape is right.** The contract is now asserted per verb in [test/remote_api_test.dart](../test/remote_api_test.dart) ("the Content-Type contract").
  - Same defect applied to `deleteWalk` and `deletePhoto`, fixed in the same pass — neither had been exercised either.
...
#### 9.3b Case-review fields — READ-ONLY, GET-list only (TB-26)
The GET list additionally returns `reviewedBy` (`TB_MOTNJE.OBRAVNAVAL`, the reviewing official's e-mail) and `reviewedAt` (`OBRAVNAVANO`, a Z-tagged UTC instant), so the warden sees **who** took the case decision and **when**, not just the `caseStatus` verdict. Absent (APEX_JSON elides NULLs, §9.4 note) until the record has actually been reviewed.
- **Seznam zapisov rows** carry two subtitle lines: date + status (TB-30), then the **obhod link** (TB-17) when `Disturbance.obhodId` resolves against `AppState.walks` — labelled `walkLabel(walk, fmt)` (name, else local start time; shared with Seznam obhodov so the two cannot drift) and tappable through to `WalkDetailScreen`. An `obhodId` that does not resolve — the walk is still local-only, or records and walks were pulled independently — renders a muted, non-tappable *"Del obhoda"* rather than a tap that dead-ends.
- **Where status appears, and it is one colour everywhere.** `recordMarkerColorForStatus` is the single source: the home map, the walk-detail map, the filter sheet's Status swatches, the detail card's status row, and the Seznam zapisov subtitle (TB-30) all call it. Anywhere the colour appears outside a map it is paired with the status **text** — a bare colour is not a label, which is the lesson of TB-26's first cut.
- **Presentation:** an `_ObravnavaCard` on `DetailScreen` — a titled block holding `caseStatus` (with the TB-27 status colour dot) plus labelled *Obravnaval* / *Obravnavano* rows, times in **local** (TB-13). It is rendered on every *saved* record, and states *"Zapis še ni bil obravnavan."* when unreviewed, so the absence is explicit. The one exception is `DetailScreen`'s **preview mode** (TB-31, §14), which hides it: on a record that does not exist yet, "not reviewed" is trivially true and is noise at the moment of commit. The v1.6.0+15 first cut showed the two values as **unlabelled pills** among the action pills — a bare e-mail and a bare date that told the warden nothing about what they were; that was the reported defect, fixed in TB-29. `caseStatus` moved into this card at the same time, so the review outcome lives in exactly one place. Tests: `test/obravnava_card_test.dart`.
- **This app never writes them.** The web backoffice (narcis-vibed NV-220) is their sole writer. They are deliberately **absent from the CRUD wire payload** — `_payload` in [remote_api.dart](../lib/data/remote_api.dart) is an explicit allowlist and omits both, so a phone-side POST/PUT cannot clobber a reviewer's decision. Regression-tested in `test/remote_api_test.dart` ("the review fields NEVER go back on the wire in a write").
- **`OPOMBA_URADNA` (the reviewer's internal note) is deliberately NOT selected or emitted.** Whether the warden who filed the record may read it is an open product decision (TB-26 half 2, `Blocked`); until it is taken the note must not cross the wire to this client.
- **Timezone:** `OBRAVNAVANO` is a TZ-naive `TIMESTAMP` holding **UTC** — the same convention as `USTVARJEN`/`CAS_OPAZOVANJA` post-TB-14, stated explicitly in narcis-vibed's own DDL — so it takes the **identical** serializer as `createdAt`. A bare `TO_CHAR` would drop the `Z` contract and a `FROM_TZ` shift would move an already-UTC value by the session offset.
- **New cross-repo rebuild hazard, in both directions.** The GET handler now SELECTs two columns `disturbance_schema.sql` does not create. A clean rebuild from that file alone leaves this app's `GET /disturbances/` failing with `ORA-00904` (previously only the *web* module broke). Both file headers now say so; the migration lives in narcis-vibed `ords/vibed_trsca_ddl.sql`.
- Deploy runbook: [tools/ords/TB-26_DEPLOY.md](../tools/ords/TB-26_DEPLOY.md). Handler-only re-publish, no DDL. Client and server are independently deployable in either order — an older build ignores the extra keys, and a newer build reads them as null until the handler ships.
...
### 10.3 Motnje layer symbology + filter (TB-6, TB-27)
**Marker symbology.** Since **TB-27** the dot **fill encodes `status obravnave`**, not age: amber `#D97706` *Odprto* · blue `#0A84FF` *V obravnavi* · green `#1B7A1B` *Zaključeno* · gray `#8E8E93` *Predano drugi službi* (and gray for any status this build doesn't know, so a fifth state added server-side still draws). `recordMarkerColorForStatus` in [basemap.dart](../lib/widgets/basemap.dart). The hex values are **deliberately identical to the web backoffice's `STATUS_COLORS`** (narcis-vibed `web/src/lib/trsca/format.ts`) so the two apps cannot disagree about what *Zaključeno* looks like — change them only in lockstep; `test/record_marker_color_test.dart` pins them. This restores parity that NV-221 broke: narcis-vibed originally coloured by age (NV-81) *for parity with this app*, then moved to status and accepted the parity loss as its stated cost.

Unchanged by TB-27, deliberately: **shape encodes authorship** (own = filled disc, teammate = ring), the **white halo** (what makes any fill legible on any basemap), and the **youngest-renders-on-top sort** in `_buildMarkers` — a stacking rule worth keeping whatever the fill encodes, and leaving it alone means the recolour cannot shift draw order.

**Both maps use this encoding.** The walk-detail map ([walk_detail_screen.dart](../lib/screens/walk_detail_screen.dart)) draws the disturbances linked to a walk and was missed in TB-27's first pass — it kept colouring by age for one release (v1.6.0+15), so the two maps briefly disagreed about the same dot. Fixed in TB-29.

**Age colouring is gone entirely (TB-29).** `AgeBucket`, `ageBucketOf`, `allAgeBuckets`, `recordMarkerColorForAge` and the filter's `ageBuckets` dimension were all removed once the map stopped encoding age: three coarse buckets were strictly worse than the **Obdobje** window sitting beside them, which filters the same axis at day granularity with a histogram. (Note for anyone reading v1.6.0-era docs: the claim that `recordMarkerColorForAge` was the Starost swatch source was wrong — the sheet hardcoded `Colors.red/orange/blue` and never called it.)

#### Filter (TB-6)
The home-map **Motnje** layer (the org's `TB_MOTNJE` records) is configured from a bottom sheet (`showMotnjeFilterSheet`, [lib/widgets/motnje_filter_sheet.dart](../lib/widgets/motnje_filter_sheet.dart)) opened by **tapping the Motnje chip itself** — the same chip→sheet pattern as the Območja layer; there is no separate "Filter" chip. The sheet's first row is a **Prikaži na zemljevidu** master switch (the layer show/hide, `_showMotnje`), and the chip is highlighted while the layer is shown. Below it are four AND-composed filter dimensions that push each change live so the map redraws behind the sheet, modelled by the pure `MotnjeFilter` ([lib/data/disturbance_filter.dart](../lib/data/disturbance_filter.dart)):
- **Status obravnave** (TB-27) — multi-select over the four CHECK-constrained states (`allCaseStatuses`), each row carrying a swatch drawn straight from `recordMarkerColorForStatus`. **This section doubles as the map legend**: it is the only place the warden can learn what a dot's colour means, so unlike Avtor/Kategorija it is *never* hidden when the loaded records happen to share one status. A record whose status is outside the four is never hidden by this dimension — silently vanishing off the map would be a poor way to learn the vocabulary grew.
...
- **Observed-date window** — an inclusive `[from, to]` range over the records' `observedAt` span, chosen with a **day-granularity** `RangeSlider` (emits on drag-end). A compact per-day **histogram** of the age/author/category-filtered records sits above the slider (`_DayHistogramPainter`, a `CustomPaint`) with the selected range highlighted, so the day-precise range is picked against the actual distribution. Defaults to the full span (a no-op).
- **Category (group)** — multi-select over the disturbance groups (*skupine*) **present in the data** (`groupsIn`); a record matches if any of its selected types is in a chosen group. Filtered at group granularity, not the 172 individual types (group is the useful filter granularity; per-type selection is the search-driven TB-12 flow). Rendered as a collapsible `ExpansionTile` with a tristate **Vse kategorije** master that toggles all on ↔ off; each group row carries the app's shared group token (tinted circle + code, `disturbanceGroupColor`/`disturbanceGroupTint`) so the filter matches the type picker / form / detail visuals. In the model `groups` is **null** when every present group is ticked (no restriction), a subset when narrowed, and an **empty set** when none are ticked (shows nothing).

The filter predicate is applied per-record in `_buildMarkers` ([home_screen.dart](../lib/screens/home_screen.dart)) when the layer is shown; `MotnjeFilter.isActive` gates the sheet's *Ponastavi* (reset) action. Filter and show state are widget-local and not persisted, matching the other layer toggles. Note: the 703 Notranjski "legacy" records were migrated into `TB_MOTNJE` (2026-05-22, the old "Zgodovina" chip removed then) and render as ordinary Motnje markers. Decluttering that historical mass was the **Starost** filter's job until TB-29 removed it; **Obdobje** now serves that purpose, and does it better — a day-granularity range over the actual distribution rather than three fixed buckets. The `showLegacy` overlay (§11) is vestigial dead code pending removal (TB-23). This filter subsumes TB-18 (year→date, reporter→author, category→group all delivered). Tests: `test/disturbance_filter_test.dart` (model, incl. the TB-27 status dimension) + `test/motnje_filter_sheet_test.dart` (sheet interactions, incl. the Status section) + `test/record_marker_color_test.dart` (the TB-27 palette lock).
...
**`syncAll` (the sync icon's tap-target).** Single entry point that runs four phases under one `_isSyncing` flag:
1. **Push pending records.** Drain `_records.where(pendingSync)` via POST `/disturbances/`. Server's idempotent-on-UUID semantics mean retries are safe. Rows also carrying `pendingDelete` are **skipped** — see the delete queue below.
...
3. **Drain pending deletes** (TB-2). `_drainPendingDeletes` DELETEs each `pendingDelete` row and purges the confirmed ones. Ordered **before** the pull on purpose: a delete confirmed after the pull would have been merged straight back in from a server list fetched moments earlier.
4. **Pull remote list and merge.** GET `/disturbances/`, build a `_lastRemoteIds` set, and merge each `RemoteDisturbance` into `_records`. Local-only `pendingSync` records are kept, `pendingDelete` rows are left untouched, and for IDs known to both sides the server wins on the record fields while cached photo `localPath`s are preserved. Photos return without `localPath` and are lazy-fetched via `ensurePhotoCached` when the user opens the detail screen.
...
- **Delete IS queued (TB-2); update still is not.** See the delete queue below. `updateRecord` remains fire-and-forget and offline edits still do not reach Oracle — and per TB-32 that path has a second, worse problem than the missing queue.
...
**Delete queue (TB-2).** `deleteRecord` does not remove the row; it marks `pendingDelete` and persists it, then drains if it can. Three pieces make that safe:
- **`AppState.records` hides `pendingDelete` rows.** Every read surface — the home map, Seznam zapisov, the counters, `DetailScreen._liveRecord` — goes through that getter, so the record is gone from the user's point of view the instant they confirm. Sync code works off `_records` and still sees it.
- **`_mergeRemoteIntoLocal` skips `pendingDelete` rows.** This is the load-bearing guard. The merge treats the server as authoritative for records known to both sides, so without an explicit branch the next pull would overwrite the local row, clear the flag, and the delete would **silently undo itself**. That was the behaviour before TB-2: the old `deleteRecord` removed the row and its photos immediately, fired DELETE with no retry, and an offline delete therefore reverted on the next pull — the row came back and its photos re-downloaded. Regression-tested in [test/record_delete_test.dart](../test/record_delete_test.dart) ("a failed delete stays queued and the pull does NOT resurrect it").
- **A 404 counts as success.** `RemoteApi.deleteRecord` already accepted `{200, 204, 404}`, which is what makes the queue safe for a row whose create never landed, and for a retry after a lost response. That is also why a `pendingSync` row queued for deletion is skipped by phase 1 rather than POSTed and then DELETEd: on a flaky link the POST could land while the DELETE does not, leaving exactly the orphan the user asked to remove.

`deleteRecord` returns `true` only when the server confirmed and the row was purged, so the UI can promise "Zapis je izbrisan" versus "Zapis bo izbrisan ob naslednji sinhronizaciji" truthfully.

**What the phone refuses to delete.** `Disturbance.isLockedByReview` is true once a record carries TB-26's `reviewedBy`/`reviewedAt` (web-backoffice-only fields) **or** its `caseStatus` has moved off the create-time `'Odprto'`. `RecordActionsMenu` ([record_list_screen.dart](../lib/screens/record_list_screen.dart)) then explains why instead of deleting — the menu item stays enabled deliberately, because a greyed-out row tells the warden nothing. `DELETE :id` is a hard cascade (row + type/observer junctions + photos, §9.3) with nothing to restore from, so a record a reviewer has already based a decision on must not be removable from the phone.
...
- **Preview before commit (TB-31).** The form's final action is *Preglej in shrani*, not a save. `FormScreen._openPreview` validates, builds the `Disturbance`, and pushes `DetailScreen(preview: true, onSave: ...)` **on top of** the form; the commit happens from that screen's *Shrani zapis*. The record is the warden's last checkpoint before it becomes read-only on the phone — corrections after the fact still mean the desktop back office until TB-2 lands.
  - **The preview route sits above the form on purpose.** `_FormScreenState` holds every field, so *Uredi* is a plain `pop` and the form returns fully populated. Popping the form and re-pushing it would look identical until a warden hit *Uredi* and lost their work.
  - **`DetailScreen` needed no data changes.** `_liveRecord` already falls back to `widget.record` when the id is absent from `AppState.records` — exactly the uncommitted case — and `_ensurePhotos` skips photos that have a `localPath` or `pendingUpload`, which every form-built photo does, so the preview issues no network calls (contrast §13's `ensurePhotoCached`, which only fires for server-side photos).
  - **Two blocks are hidden in preview:** `_SyncBadge` (the form sets `pendingSync: true`, so it would announce "queued to sync" for a record the server has never seen) and `_ObravnavaCard` (§ TB-26 above).
  - **Save is guarded against double-taps.** `DetailScreen._handleSave` sets a `_saving` flag and disables the button for the whole await. `AppState.addRecord` awaits photo materialisation, the optimistic POST *and* the photo uploads — seconds on a weak link — and the old save button stayed live throughout, so a second tap minted a second UUID and filed a second record. That is the leading candidate for the duplicate field records reported in TB-2.
  - Tests: [test/preview_before_save_test.dart](../test/preview_before_save_test.dart) (preview mode, the double-tap, narrow-screen layout), [test/form_preview_flow_test.dart](../test/form_preview_flow_test.dart) (the round trip, and that *Uredi* returns to a filled form).
