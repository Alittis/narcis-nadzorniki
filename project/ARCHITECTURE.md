...
**Historical walks layer.** A "Obhodi" chip in the layer row toggles a blueGrey polyline overlay for completed walks. Renders any walk with non-empty `points`. Coverage policy: `syncAll` calls `_prefetchOwnWalkPoints` after `_pullRemoteWalks`, which downloads geometry for every walk the caller authored (`createdBy == currentUser`) that doesn't yet have points cached. Teammate walks stay lazy — they only appear in this layer after the user opens them in the detail view (which calls `ensureWalkPointsCached`). The asymmetry scopes bandwidth to the user's own activity rather than the org's: a 2-hour walk's points payload is ~150 KB, so eager-fetching every org walk on every device would compound on cellular. Failures during prefetch are tolerated per-walk (one bad fetch doesn't block the rest).
