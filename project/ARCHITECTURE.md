...
Unchanged by TB-27, deliberately: **shape encodes authorship** (own = filled disc, teammate = ring), the **white halo** (what makes any fill legible on any basemap), and the **youngest-renders-on-top sort** in `_buildMarkers` — a stacking rule worth keeping whatever the fill encodes, and leaving it alone means the recolour cannot shift draw order. `ageBucketOf` and the **Starost** filter both stay; `recordMarkerColorForAge` survives as the swatch source for that filter's rows, which is what keeps those swatches and the bucket thresholds from drifting.

#### Filter (TB-6)
The home-map **Motnje** layer (the org's `TB_MOTNJE` records) is configured from a bottom sheet (`showMotnjeFilterSheet`, [lib/widgets/motnje_filter_sheet.dart](../lib/widgets/motnje_filter_sheet.dart)) opened by **tapping the Motnje chip itself** — the same chip→sheet pattern as the Območja layer; there is no separate "Filter" chip. The sheet's first row is a **Prikaži na zemljevidu** master switch (the layer show/hide, `_showMotnje`), and the chip is highlighted while the layer is shown. Below it are five AND-composed filter dimensions that push each change live so the map redraws behind the sheet, modelled by the pure `MotnjeFilter` ([lib/data/disturbance_filter.dart](../lib/data/disturbance_filter.dart)):
- **Age** — three buckets keyed off `ageBucketOf` (≤ 31 d → red, ≤ 365 d → orange, older → blue). The row swatches come from `recordMarkerColorForAge`, the *same* function the thresholds live in, so swatches and buckets can never drift. Since TB-27 these three colours appear **only here** — they no longer encode anything on the map.
