...
Implemented in [lib/widgets/funding_footer.dart](../lib/widgets/funding_footer.dart) as `FundingFooter`, rendered at the bottom of the Profil page ([profile_screen.dart](../lib/screens/profile_screen.dart)): three logos in a row (LIFE / Natura 2000 / Life Tršca project) + the Slovenian funding sentence. Last item of the Profil ListView, below the Odjava (logout) row, separated by a Divider. Login screen was tried in v1.2.1+8 but the logos looked visually mismatched against the photo background; reverted in v1.2.2+9 to Profil-only.

Logos are bundled as PNGs at `assets/images/funding_life.png`, `assets/images/funding_natura2000.png`, `assets/images/funding_life_trsca.png`. Source PDFs (publisher-supplied) are NOT committed. Rasterisation pipeline (one-time, manual): `pdftoppm -png -scale-to-y 600 <pdf> <out>` then `magick <out>.png -trim +repage <out>.png` to crop the publisher-baked white/blue padding around each logo so the three PNGs occupy bounding boxes that match their actual visible content. Without the `-trim` step the LIFE logo had ~290 px of blue padding on each side and Life Tršca had padding all around; their effective aspect ratios in the UI looked squeezed relative to Natura 2000 (which natively filled its canvas).

Aspect ratios after trim: LIFE 2.00:1 (square-ish landscape), Natura 2000 0.40:1 (portrait), Life Tršca 3.02:1 (wide landscape). Layout is a `Row` of three `Expanded` cells so each logo gets equal horizontal real estate; `BoxFit.contain` lets each logo size itself within its column without distortion. Row is height-capped at 56 px.
