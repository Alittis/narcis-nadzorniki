...
- **Layer picker**: the "Območja" chip opens `showObmocjaPicker` (bottom sheet, `lib/widgets/obmocja_picker.dart`) with one checkbox per sublayer (colour dot + name); N2k on by default. The active set is `_activeZos` (`Set<ZosKind>`, widget-local, not persisted); toggling redraws the map live, and the chip reads "selected" whenever the set is non-empty.
...
- **Sheet**: `showObmocjaSheet` lists the hits (per-kind colour dot + name + kind/code); a row-tap drills into that area's detail (a single hit opens detail directly). N2k keeps its POV/POO red/orange badge; other kinds use their sublayer label + colour. All data is already in the GetFeatureInfo response — the sheet does no further fetching.
...
- Implementation: `lib/data/obmocja_store.dart` (`ZosKind`, `zosWmsLayers`/`zosOrder`, `identify` + `ObmocjeFeature`), `obmocjaWmsLayers()` in `lib/widgets/basemap.dart`, `lib/widgets/obmocja_picker.dart` (picker), `lib/widgets/obmocje_sheet.dart` (list ⇄ detail + per-kind presentation), wired in `home_screen.dart` (`_activeZos`, `_identifyObmocja`, the "Območja" chip). Unit tests in `test/obmocja_store_test.dart` (request shape + per-kind multi-sublayer parse). `proj4dart` and the GeoJSON parsing were removed with the vector approach.
