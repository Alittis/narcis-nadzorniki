...
**Capture accuracy mode + render-time polish (TB-3, 2026-06-22).** The handler stream now requests `LocationAccuracy.bestForNavigation` (was `high`) — on iOS this forces the navigation GNSS mode; on Android it still rides the fused provider, so it's a nudge, not a cure. The capture-time gate above stays at **50 m** (it only rejects obvious non-GNSS fixes; the raw points are the honest, write-once record). The real defence against "scattered off the path" tracks is a **render-time** pass: `polishTrack` ([lib/services/track_polish.dart](../lib/services/track_polish.dart)) drops fixes worse than **20 m** accuracy and applies a small centred moving average to the survivors, applied wherever a track is *drawn* (home-map active + historical polylines, walk-detail map) — never to stored data, so the thresholds stay tunable. Grounding: real walks pulled via ORDS are bimodal — clean walks sit at a median 4–8 m (p90 ≤ 10 m, untouched by the 20 m cut), while the occasional walk rides 20–50 m for its whole length (a phone stuck on coarse/fused location, not random jitter). The 20 m cut keeps ≈100 % of clean-walk fixes and ≈40 % of a pathological walk's, then smooths the rest. Unit-tested in [test/track_polish_test.dart](../test/track_polish_test.dart).
