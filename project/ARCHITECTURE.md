...
### 10.2 Overlay: Protected Areas (Natura 2000, vector)
Optional vector overlay of Natura 2000 protected areas ("Območja s statusom"), toggled by the "Območja" chip in the home-map layer row. Unlike Parcele (server-rendered WMS raster tiles), this layer is **client-rendered vector** — fetched as GeoJSON, parsed, reprojected, and drawn as `flutter_map` polygons — so taps can identify the area under the finger.

- **Source**: public NarcIS ORDS GIS modules on `narcis.gov.si` (no auth), shared upstream with the `narcis-vibed` web app. Single-host by design — the field app talks only to `narcis.gov.si`, same as its disturbance/walk data. (We deliberately do **not** depend on the `narcis-vibed` deployment at `narcis.alittis.com`, even though its `/api/layers` route serves pre-reprojected GeoJSON, to avoid coupling a Play-Store field app to the web app's uptime.)
  - Geometry: `GET /ords/narcis/vib/zos/N2k` → ORDS envelope `{items:[{geojson:"<stringified FeatureCollection>"}]}` in **EPSG:3794** (Slovene national grid). ~3.8 MB, 356 features (325 Polygon + 30 MultiPolygon + 1 Point).
  - Detail: `GET /ords/narcis/vib/zos-detail/:id` → `{items:[{detail:"<stringified attributes>"}]}` (name, code, status, description, area in ha, dates, biogeo region).
- **CRS**: source is EPSG:3794; reprojected to WGS84 client-side via `proj4dart` (`+proj=tmerc … lon_0=15 … x_0=500000 y_0=-5000000 ellps=GRS80`, the same definition `narcis-vibed` uses). Reprojection runs once at load.
- **Styling** (matches `narcis-vibed`): POO (SAC) = translucent orange fill `rgba(217,130,16,0.40)` + grey hairline; POV (SPA) = light red fill + solid red `#e30000` outline. `narcis-vibed` draws POV as a diagonal hatch, which `flutter_map`'s CPU polygon painter cannot reproduce — the solid red outline keeps the two designations clearly distinct.
- **Loading**: lazy — fetched on first "Območja" enable (online-only; a snackbar reports offline / failure), then cached for the app session in `ObmocjaStore`. Toggle state `_showObmocja` is widget-local, not persisted.
- **Tap-to-identify**: `MapOptions.onTap` runs an even-odd ray-cast (`areaAtPoint`) over loaded polygons (holes ignored — acceptable for a tap test); a hit opens a bottom sheet (`showObmocjeSheet`) that lazily fetches the detail endpoint and degrades to inline name/code/tip when offline.
- **Scope**: N2k only for now. The same `vib/zos/:tip` endpoint serves `ZO`, `EPO`, `NV`, `NVJ`; those are deferred — `NV` (5.3k features) and `NVJ` (14.7k, cave points) need viewport-loading or marker clustering before they're viable as on-device vector layers.
- **Known perf risk**: 356 dense multipolygons is the heaviest the home map has drawn; `PolygonLayer(polygonCulling: true)` drops off-screen polygons. If pan/zoom stutters on mid-range devices, levers are client-side Douglas–Peucker simplification, a zoom gate, or moving parse+reproject to a background isolate (`compute`).
- Implementation: `lib/data/obmocja_store.dart` (fetch + parse + reproject + hit-test), `obmocjaLayers()` in `lib/widgets/basemap.dart` (polygons + point discs), `lib/widgets/obmocje_sheet.dart` (detail sheet), wired in `home_screen.dart` (`_showObmocja`, `_toggleObmocja`, the "Območja" chip). Unit tests in `test/obmocja_store_test.dart` (envelope parse, golden 3794→WGS84 reprojection, caching, ray-cast).
